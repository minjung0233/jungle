...
int smallestValue(BTNode *node)
{
    ...
    // 1. 현재 node와 양쪽 노드 비교
    // 2-1.왼쪽 기준 : 현재 노드보다 더 작으면 갱신
    int value = smallestValue(node->left);
    if(node->item > value){
        return value;
    }else{
        return node->item;
    }
    // 2-1.오른쪽 기준 : 현재 노드보다 더 작으면 갱신
    value = (value < smallestValue(node->right)) ? smallestValue(node->right) : value;
    if(node->item > value){
        return value;
    }else{
        return node->item;
