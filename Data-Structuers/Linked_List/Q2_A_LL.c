...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
	// ll1과 ll2 중 비교해서 작은 수(length) 만큼 돌면서 
	// removeNode(ll2, i)해서 가져오고 넣기
	int length;
	...
	for (int i=0; i<length; i++){	// 0,1,2 -> 1,2,3
		ListNode *temp=NULL;
		int index;
		// 0, 1, 2
		temp = findNode(ll2, 0);
		...
		if(temp != NULL){
			...
			// removeNode(&ll2, i)를 ll1에 insert
			// 홀수만큼 추가 - 0->1, 1->3, 2->5..
			index = i*2+1;
			insertNode(ll1,index,temp->item);	// ll1의 홀수번째에 temp 값 넣기
			removeNode(ll2, 0);		// ll2의 index 0번째 지우기 
